...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	$row->AddViewField("SESSION_ID", getLink("iggle_stat_sess.php?set_filter=Y&find_ID=".$f_SESSION_ID."&lang=".LANG, $f_SESSION_ID, false));
	$row->AddViewField("PAGE_TITLE", getLink($f_URI, $f_PAGE_TITLE));
	$row->AddViewField("IP", GetWhoisLink($f_IP));
}

$lAdmin->AddAdminContextMenu();
$lAdmin->CheckListMode();
$APPLICATION->SetTitle(GetMessage("IGGLE_STAT_MENU_HIT_ALT"));
...
$oFilter = new CAdminFilter(
		...
		array(
			...
			$arTableFields["SESSION_ID"]["title"],
...
?>
		...
		<tr>
			<td><?=$arTableFields["SESSION_ID"]["title"]?>:</td>
			<td><input type="text" name="find_SESSION_ID" value="<?echo htmlspecialchars($find_SESSION_ID)?>"></td>
		</tr>
		<tr>
